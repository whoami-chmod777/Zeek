...
void Manager::InitPostScript() {
    ...
    if ( logSeverityVal < 0 || logSeverityVal > static_cast<int>(BrokerSeverityLevel::debug) )
...
void Manager::ProcessLogEvents() {
    ...
    auto evType = [](BrokerSeverityLevel lvl) {
        switch ( lvl ) {
            case BrokerSeverityLevel::critical: return ev_critical.get();
            case BrokerSeverityLevel::error: return ev_error.get();
            case BrokerSeverityLevel::warning: return ev_warning.get();
            case BrokerSeverityLevel::info: return ev_info.get();
            case BrokerSeverityLevel::verbose: return ev_verbose.get();
            default: return ev_debug.get();
        }
    };
