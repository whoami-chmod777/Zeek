...
const IDPtr& lookup_ID(const char* name, const char* curr_module, bool no_global,
	...
	// tweak {ID} to reject GLOBAL::, or switch this warning to a descriptive
	// error instead.
	if ( zeek::util::starts_with(name, "GLOBAL::") )
		reporter->Deprecation(util::fmt("Remove in v7.1: Use :: instead of GLOBAL:: (%s)", name));

	std::string fullname = make_full_var_name(curr_module, name);
	std::string ID_module = extract_module_name(fullname.c_str());

	if ( explicit_global && ID_module.empty() )
		ID_module = GLOBAL_MODULE_NAME; // Internally, we continue to prefix with GLOBAL
